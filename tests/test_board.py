...
class TestDotCount:
    def test_week_granularity(self):
        ...
        assert 1 == board.task_dot_count(
            history, MID_SATURDAY - DAY, board.WEEKLY_DOTS
        )
        history = [
            {'ts': MID_SATURDAY - ((8*DAY)-HOUR), 'column': 'In Progress'},
        ]
        assert 0 == board.task_dot_count(
            history, MID_SATURDAY - DAY, board.WEEKLY_DOTS
        )
    ...
    def test_day_granularity(self):
        ...
        assert 1 == board.task_dot_count(history, MID_SATURDAY - DAY)
        history = [
            {'ts': MID_SATURDAY - (2*DAY - HOUR), 'column': 'In Progress'},
        ]
        assert 0 == board.task_dot_count(history, MID_SATURDAY - DAY)
    ...
    def test_multiple_column_stints(self):
        ...
        assert 2 == board.task_dot_count(history, MID_SATURDAY-DAY)
...
class TestHistory:
    def test_creation_only(self):
        ...
        assert board.task_history(board_name, stories) == expected_history
